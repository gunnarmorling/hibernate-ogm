...
//* commands there. As long as that's the case we cannot really derive the intended export target from that.
//* {@code SchemaExport} is in a transitional state as per Steve, so we should be able to do the right thing down the
//* road. For now, only DB export will be supported, but no file exports (if feasible with given stores anyways, e.g. for
//* MongoDB it will be hard to come up with sensible file exports; We could create JavaScript commands representing
//* "DDL", but it'd be tough to re-import and execute them later on).
